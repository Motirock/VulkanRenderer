...
/* Quick Use
glslc shaders/blocks.vert -o shaders/blocks.vert.spv && glslc shaders/blocks.frag -o shaders/blocks.frag.spv && glslc shaders/ocean.vert -o shaders/ocean.vert.spv && glslc shaders/ocean.frag -o shaders/ocean.frag.spv   && glslc shaders/skybox.vert -o shaders/skybox.vert.spv && glslc shaders/skybox.frag -o shaders/skybox.frag.spv && g++ -c -std=c++17 -O3 src/*.cxx -Iinclude && g++ *.o -o bin/main -Llib -lglfw -lvulkan -ldl -lpthread -lX11 -lXxf86vm -lXrandr -lXi && ./bin/main
*/
...
private:
    ...
    VkRenderPass secondaryRenderPass;
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            if (time*TPS >= ticks) {
                ...
                if (ticks % TPS == 0) {
                    ...
                    uint32_t waterPolygonCount = 2*WORLD_CHUNK_SIZE_X*WORLD_CHUNK_SIZE_Y*CHUNK_SIZE*CHUNK_SIZE*2*2;
                    ...
                    std::cout << "Frame time: " << averageFrameTime <<
                        ...
                        << "Polygons rendered: " << indexCount/3*instanceCount+waterPolygonCount << "\n"
    ...
    void cleanup() {
        ...
        vkDestroyRenderPass(device, primaryRenderPass, nullptr);
        vkDestroyRenderPass(device, secondaryRenderPass, nullptr);
    ...
    void createPrimaryRenderPass() {
        ...
        if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &primaryRenderPass) != VK_SUCCESS) {
    ...
    void createSecondaryRenderPass() {
        ...
        if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &secondaryRenderPass) != VK_SUCCESS) {
            throw std::runtime_error("Failed to create render pass!");
        }
    }
    ...
    void createBlocksPipeline() {
        ...
        pipelineInfo.renderPass = primaryRenderPass;
    ...
    void createOceanPipeline() {
        ...
        pipelineInfo.renderPass = secondaryRenderPass;
        pipelineInfo.subpass = 0;
        pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;
    ...
    void createFramebuffers() {
        ...
        for (size_t i = 0; i < swapChainImageViews.size(); i++) {
            ...
            framebufferInfo.renderPass = primaryRenderPass;
    ...
    void createOceanBuffers() {
        std::vector<PositionVertex> vertices;

        glm::vec2 worldDimensions = glm::vec2(WORLD_CHUNK_SIZE_X*CHUNK_SIZE, WORLD_CHUNK_SIZE_Y*CHUNK_SIZE);
        float seaLevel = 32.0f;
        
        for (int j = 0; j < worldDimensions.y*2; j++) {
            for (float i = 0; i < worldDimensions.x*2; i++) {
                float x = i/2.0f;
                float y = j/2.0f;

                float widthX = 1.0f/2.0f;
                float widthY = 1.0f/2.0f;

                vertices.push_back({{x+widthX, y+widthY, seaLevel}});
                vertices.push_back({{x,        y+widthY, seaLevel}});
                vertices.push_back({{x,        y,        seaLevel}});

                vertices.push_back({{x,        y,        seaLevel}});
                vertices.push_back({{x+widthX, y,        seaLevel}});
                vertices.push_back({{x+widthX, y+widthY, seaLevel}});
            }
        ...
        //Positive z (top)
        // vertices.push_back({{ worldDimensions.x, -worldDimensions.y,  seaLevel}});
        // vertices.push_back({{-worldDimensions.x,  worldDimensions.y,  seaLevel}});
        // vertices.push_back({{ worldDimensions.x,  worldDimensions.y,  seaLevel}});
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        renderPassInfo.renderPass = primaryRenderPass;
        ...
        VkRenderPassBeginInfo renderPassInfo{};
        renderPassInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
        renderPassInfo.renderPass = secondaryRenderPass;
        renderPassInfo.framebuffer = swapChainFramebuffers[imageIndex];
        renderPassInfo.renderArea.offset = {(int32_t) swapChainExtent.width/4, (int32_t) swapChainExtent.height/4};
        renderPassInfo.renderArea.extent = {swapChainExtent.width/2, swapChainExtent.height/2};
        ...
        vkCmdBeginRenderPass(commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
            ...
            vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, oceanPipelineLayout, 0, 1, &descriptorSets[currentFrame], 0, nullptr);
			vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, oceanPipeline);
			vkCmdBindVertexBuffers(commandBuffer, 0, 1, &oceanVertexBuffer, offsets);

            vkCmdDraw(commandBuffer, static_cast<uint32_t>(6*WORLD_CHUNK_SIZE_X*WORLD_CHUNK_SIZE_Y*CHUNK_SIZE*CHUNK_SIZE*2*2), 1, 0, 0);         
            }
