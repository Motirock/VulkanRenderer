...
const std::string TEXTURE_ATLAS_PATH = "textures/viking_room.png";
...
private:
    ...
    std::vector<VkFence> defferedFences;
    ...
    void cleanup() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            vkDestroyFence(device, defferedFences[i], nullptr);
        }
    ...
    void createSyncObjects() {
        ...
        defferedFences.resize(MAX_FRAMES_IN_FLIGHT);
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS ||
                ...
                vkCreateFence(device, &fenceInfo, nullptr, &defferedFences[i]) != VK_SUCCESS) {
                throw std::runtime_error("Failed to create synchronization objects for a frame!");
            }

            vkResetFences(device, 1, &defferedFences[i]);
        }
    }
    ...
    void drawFrame() {
        ...
        if (vkQueueSubmit(graphicsQueue, 1, &submitInfo, defferedFences[currentFrame]) != VK_SUCCESS) {
            throw std::runtime_error("Failed to submit draw command buffer for off screen render!");
        }
        }

        {
        vkWaitForFences(device, 1, &defferedFences[currentFrame], VK_TRUE, UINT64_MAX);

        vkResetFences(device, 1, &defferedFences[currentFrame]);
