...
layout(binding = 1) uniform sampler2D gSamplers[12];
...
vec3 getFinalColor(int samplerStartingIndex, in vec2 textureCoordinates, vec3 viewPosition) {
    ...
    if (samplerStartingIndex == 0)
        return hdrColor;
    return hdrColor+texture(gSamplers[samplerStartingIndex+3], textureCoordinates).xyz;
}
...
void main() {
    ...
    if (tempColor.w != 0.0f || tempPosition.z > 5.0f) {
        vec3 finalColor = getFinalColor(0, fragmentTextureCoordinates, ubo.cameraPosition);
        vec3 bloom = texture(gSamplers[3], fragmentTextureCoordinates).xyz;
        outColor = vec4(toneMap(finalColor+bloom), 1.0);
        return;
    }
    else {
        ...
        vec3 reflect = getFinalColor(4, reflectionTextureCoordinates, tempPosition.xyz)*((1.0f-abs(normalize(tempPosition.xyz-ubo.cameraPosition).z))*0.9f+0.1f);
        vec3 refract = getFinalColor(8, refractionTextureCoordinates, tempPosition.xyz)*(abs(normalize(tempPosition.xyz-ubo.cameraPosition).z)*0.9f+0.1f);
